...
| `POST /api/v1/documents/{id}/acl` | `grant` | Grant an invited eIDAS serial standing read + co-sign access to a document's chain (held only by the workflow service; no self-grant). The serial is stored in the platform's canonical spelling; one naming no country is refused `422`. Idempotent. |
...
- **eIDAS serial matching is on one canonical spelling.** A grant and a co-signer's own identity code are each reduced to the identity type, the country, a hyphen and the national code with its separators removed, so ACL matching is plain equality however each side writes the code — and the country stays part of the key, so the same digits in another country are another person. A grant naming a code with no country is refused (`422 err:document:invalidSerial`): the workflow service that calls this has already resolved one from the person it invited, so a bare code here is a fault to name, not a nationality to invent.
